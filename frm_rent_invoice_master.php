            ...
            if ($row) {
              ...
              if ($generator_options) {
                ...
                if (is_array($fields_names) && !empty($fields_names)) {
                  for ($i = 0; $i < count($fields_names); $i++) {
                   ...
                   if ($fieldname == "invoice_type") {
                    ...
                    echo '<input type="number" id="' . $fieldname . '" name="' . $fieldname . '" class="form-control duplicate" placeholder="Enter Rent Invoice Sequence" value="' . htmlspecialchars($next_invoice_sequence) . '" required>';
                ...
                if ($fieldname == "invoice_no" && $side_by_side_started) {
                    echo $lbl_str;
                    echo '<div class="col-6 col-sm-2 col-md-1 col-lg-1">';
                    echo '<input type="text" id="invoice_no" name="invoice_no" class="form-control duplicate" placeholder="Invoice No" value="' . htmlspecialchars($invoice_no_formatted) . '" required disabled>';
                    echo '<div class="invalid-feedback"></div>';
                echo '<input type="hidden" id="invoice_no_hidden" name="invoice_no" value="' . htmlspecialchars($invoice_no_formatted) . '">';
                    echo $error_container;
                    echo '</div>';
                    echo '</div>';
                    $side_by_side_started = false;
...
// On page load: set default value from PHP variable (if any)
(function() {
...
// --- ADD THIS: Auto-copy invoice date to billing date on TAB/out ---
if (invoiceDateInput && billingTillDateInput) {
    ...
    // Existing validation listeners (keep these)
...
function validateInvoiceDate(dateString) {
    ...
    if (companyYearStart && companyYearEnd) {
        ...
        const currentFYStart = new Date(2025, 3, 1); // April 1, 2025
        const currentFYEnd = new Date(2026, 2, 31);  // March 31, 2026
