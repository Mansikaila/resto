...
$('#customer').on('change', function() {
    ...
    // Handle hsncode change to fetch tax rates-drashti
...
function updateTableRow(index, rowData) {
 ...
 function updateHSNCodeByInvoiceType() {
    ...
    if (selectedType && hsnDropdown) {
        ...
        // --- Trigger the change event so tax rates are filled ---
        // Handle hsncode change to fetch tax rates-drashti
...
document.getElementById("generate").addEventListener("click", function () {
    ...
    $.ajax({
        ...
        success: function (invoiceData) {
            ...
            // --- SET THE TOTAL IN BASIC AMOUNT FIELD ---
...
// Helper function to get the float value or 0
function getFloat(id) {
  var v = document.getElementById(id);
  return v && v.value ? parseFloat(v.value) || 0 : 0;
}

// Recalculate basic_amount and update taxes
function recalculateInvoiceAmounts() {
  // Get grid total
  let gridTotal = getFloat("basic_amount"); // This is set from the grid already

  // Add other expenses if present
  let unloading = getFloat("unloading_exp");
  let loading = getFloat("loading_exp");
  let other3 = getFloat("other_expense3");

  // Calculate new basic amount
  let newBasic = gridTotal + unloading + loading + other3;
  // Set the new total in the basic_amount field
  let basicInput = document.getElementById("basic_amount");
  if (basicInput) basicInput.value = newBasic.toFixed(2);

  // Calculate taxes based on which radio is selected
  ...
  // Clear amount fields first
  if (sgstAmtInput) sgstAmtInput.value = "";
  if (cgstAmtInput) cgstAmtInput.value = "";
  if (igstAmtInput) igstAmtInput.value = "";
  ...
  // Calculate and set values ONLY in *_amt fields
  if (taxType == "2" && sgstAmtInput && cgstAmtInput) { // SGST+CGST
    sgstAmtInput.value = (newBasic * 0.09).toFixed(2);
    cgstAmtInput.value = (newBasic * 0.09).toFixed(2);
  } else if (taxType == "3" && igstAmtInput) { // IGST
    igstAmtInput.value = (newBasic * 0.18).toFixed(2);
  } else if (taxType == "1" && cgstAmtInput) { // Only CGST
    cgstAmtInput.value = (newBasic * 0.12).toFixed(2);
  }
}

// Attach to events
["unloading_exp", "loading_exp", "other_expense3"].forEach(function(id) {
  let el = document.getElementById(id);
  if (el) el.addEventListener("input", recalculateInvoiceAmounts);
});
// Also run when tax type changes
...
// Optionally, also recalculate after invoice grid is generated
var generateBtn = document.getElementById("generate");
if (generateBtn) {
  generateBtn.addEventListener("click", function() {
    setTimeout(recalculateInvoiceAmounts, 200); // Wait for grid to update value
  });
}

// Also recalculate when page loads (to initialize)
