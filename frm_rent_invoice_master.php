...
<section class="content">
 ...
 if ($fieldname == "lot_no") {
    $selected_lot_nos = [];
    if ($transactionmode == "U" && !empty($_bll->_mdl->_detail_records)) {
        // Extract lot_no values from detail records
        $selected_lot_nos = array_column($_bll->_mdl->_detail_records, 'lot_no');
    }
    $selected_lot_nos_json = json_encode($selected_lot_nos);
?>
    ...
    <div class="col-8 col-sm-4 col-md-3 col-lg-2">
        ...
        <script>
            // Pass selected lot numbers to JavaScript
            var preSelectedLotNos = <?php echo $selected_lot_nos_json; ?>;
        </script>
    </div>
<?php
...
function setLotNoOptions(lots, error = false) {
    ...
    let allCheckbox = `
        <label for="lot_no_all">
            <input type="checkbox" id="lot_no_all" onchange="toggleAllLotNoCheckboxes(this)" />
            All
        </label>
    `;
    let lotsCheckboxes = lots.map((lot, idx) => {
        // Check if the lot_no is in preSelectedLotNos
        const isChecked = preSelectedLotNos.includes(lot.lot_no) ? 'checked' : '';
        return `<label for="lot_no_${idx}">
            <input type="checkbox" id="lot_no_${idx}" value="${lot.lot_no}" onchange="lotNoCheckboxStatusChange()" name="lot_no[]" ${isChecked}/>
            ${lot.lot_no}
        </label>`;
    }).join('');
