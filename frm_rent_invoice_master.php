...
<div id="generatedInvoiceGrid" class="mt-4" style="<?php echo ($transactionmode == "U") ? '' : 'display:none;' ?>">
...
<!--                <th>In. No.</th>-->
        ...
        <tbody id="generatedInvoiceTableBody">
            <?php if ($transactionmode == "U"): ?>
                <!-- Load existing generated details here -->
                <?php 
                $generatedDetails = [];
                try {
                    $stmt = $_dbh->prepare("SELECT * FROM tbl_rent_invoice_detail WHERE rent_invoice_id = ? AND inward_no IS NOT NULL");
                    $stmt->execute([$_bll->_mdl->_rent_invoice_id]);
                    $generatedDetails = $stmt->fetchAll(PDO::FETCH_ASSOC);
                } catch (PDOException $e) {
                    error_log("Error loading generated details: " . $e->getMessage());
                }
                ...
                <?php else: ?>
               <?php foreach ($generatedDetails as $detail): ?>
    <tr>
        <td><?php echo isset($detail['inward_date']) && $detail['inward_date'] !== null ? $detail['inward_date'] : ''; ?></td>
        <td><?php echo htmlspecialchars($detail['lot_no'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['item'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['marko'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['invoice_qty'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['unit_name'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['wt_per_kg'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['storage_duration'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['rent_per_storage_duration'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['rent_per'] ?? ''); ?></td>
        <td><?php echo isset($detail['outward_date']) && $detail['outward_date'] !== null ? $detail['outward_date'] : ''; ?></td>
        <td><?php echo isset($detail['charges_from']) && $detail['charges_from'] !== null ? $detail['charges_from'] : ''; ?></td>
        <td><?php echo isset($detail['charges_to']) && $detail['charges_to'] !== null ? $detail['charges_to'] : ''; ?></td>
        <td><?php echo htmlspecialchars($detail['actual_month'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['actual_day'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['invoice_for'] ?? ''); ?></td>
        <td><?php echo htmlspecialchars($detail['invoice_day'] ?? ''); ?></td>
        <td><?php echo number_format($detail['invoice_amount'] ?? 0, 2); ?></td>
...
document.addEventListener("DOMContentLoaded", function () {    
    ...
    const masterForm = document.getElementById("masterForm");
     <?php if ($transactionmode == "U" && !empty($generatedDetails)): ?>
        generatedDetailsData = <?= json_encode($generatedDetails) ?>;
        
        // Also populate the jsonData array with manual entries
        <?php 
        $manualEntries = [];
        try {
            $stmt = $_dbh->prepare("SELECT * FROM tbl_rent_invoice_detail WHERE rent_invoice_id = ? AND (inward_no IS NULL OR inward_no = '')");
            $stmt->execute([$_bll->_mdl->_rent_invoice_id]);
            $manualEntries = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error loading manual entries: " . $e->getMessage());
        }
        ?>
        jsonData = <?= json_encode($manualEntries) ?>;
    <?php endif; ?>
