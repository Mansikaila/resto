<?php
    ...
    include("from_item_customer_dropdown.php");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($itemId && $customerId) {
        ...
        echo "<div class='alert alert-warning'>Please select both Customer and Item.</div>";
    }
    exit;
}
...
$(document).ready(function () {
    // Fetch packing units based on both item and customer selection
$('#item_id, #customer_id').on('change', function() {
    ...
    if (itemId && customerId) {
        $.ajax({
            ...
            error: function() {
                $('#searchDetail .box-detail').html('<div class="alert alert-danger">Error loading data</div>');
            }
        });
    } else {
        $('#searchDetail .box-detail').html('<div class="alert alert-warning">Please select both Customer and Item</div>');
    }
});
    ...
    $('#btn_add').on('click', function (event) {
        ...
        gridRows.each(function () {
            ...
            const rentPerQtyMonth = monthCell.text().trim();
            const rentPerQtySeason = seasonCell.text().trim();

            const originalMonth = monthCell.data('original')?.toString().trim() || '';
            const originalSeason = seasonCell.data('original')?.toString().trim() || '';

            // If values haven't changed, skip this row
            if (rentPerQtyMonth === originalMonth && rentPerQtySeason === originalSeason) {
                return;
            }

            // Basic validation
            if (!packingUnitId || rentPerQtyMonth === "" || rentPerQtySeason === "") {
                alert('All fields are required in the grid!');
                hasError = true;
        ...
        // Send serialized details to hidden input
        $('#detail_records').val(JSON.stringify(detailRecords));

        // Submit the form to be processed in PHP
        $('#masterForm').submit();
    });
});
</script>  
