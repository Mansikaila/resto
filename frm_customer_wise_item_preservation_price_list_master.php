    ...
    document.getElementById("btn_add").addEventListener("click", function () {
        ...
        setTimeout(function () {
            ...
            if (invalidInputs.length === 0) {
                ...
                let message = transactionMode === "U" ? "Record updated successfully!" : "Record added successfully!";
                let title = transactionMode === "U" ? "Update Successful!" : "Save Successful!";
                let icon = "success";

                (async function () {
                    let result = await Swal.fire(title, message, icon);
                    ...
                    if (data.length > 0) {
                        ...
                        data.forEach(unit => {
                            originalValues[unit.packing_unit_id] = {
                                rent_kg_per_month: unit.rent_kg_per_month,
                                season_rent_per_kg: unit.season_rent_per_kg
                            };

                            html += `
                                <tr data-id="${unit.packing_unit_id}" class="${unit.is_updated ? 'updated-row' : ''}">
                        ...
                        gridBody.querySelectorAll(".editable").forEach(cell => {
                            cell.addEventListener("input", function () {
                                ...
                                if (this.classList.contains("rent-monthly")) {
                                    modifiedRows[packingUnitId].rent_kg_per_month = this.innerText.trim();
                                } else if (this.classList.contains("rent-seasonal")) {
                                    modifiedRows[packingUnitId].season_rent_per_kg = this.innerText.trim();
                                }
                            });
                        });
                    } else {
    ...
    document.getElementById("btn_add").addEventListener("click", function (e) {
        ...
        for (const packingUnitId in modifiedRows) {
            updates.push({
                ...
                rent_kg_per_month: modifiedRows[packingUnitId].rent_kg_per_month || originalValues[packingUnitId].rent_kg_per_month,
                season_rent_per_kg: modifiedRows[packingUnitId].season_rent_per_kg || originalValues[packingUnitId].season_rent_per_kg
            });
        }
        ...
        .then(data => {
            if (data.success) {
                ...
                for (const packingUnitId in modifiedRows) {
                    delete modifiedRows[packingUnitId];
                }
            } else {
                Swal.fire({
                    icon: 'error',
