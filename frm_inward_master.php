...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const inwardWeightField = document.getElementById("inward_wt");

    if (unitField && inwardWeightField) {
        unitField.addEventListener("change", function () {
            const packingUnitId = unitField.value;

            if (packingUnitId) {
                fetch(`classes/cls_inward_master.php?action=fetchConversionFactor&packing_unit_id=${encodeURIComponent(packingUnitId)}`)
                    .then(response => response.json())
                    .then(data => {
                        console.log("Fetched data:", data); // Debugging log
                        if (data.success) {
                            inwardWeightField.value = data.conversion_factor || "0";
                        } else {
                            alert(data.message || "Conversion factor not found.");
                            inwardWeightField.value = "0";
                        }
                    })
                    .catch(error => {
