...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const inwardWtInput = document.getElementById("inward_wt");

    // Fetch and update conversion factor when packing unit is selected
        ...
        packingUnitDropdown.addEventListener("change", function () {
            ...
            if (selectedPackingUnit) {
                fetch("classes/cls_inward_master.php?action=fetchConversionFactor&packing_unit=" + encodeURIComponent(selectedPackingUnit))
                    ...
                    .then(data => {
                        if (data.success) {
                            avgWtPerBagInput.value = data.conversion_factor;
                            updateInwardWeight(); // Automatically update inward weight
                        } else {
                    ...
                    .catch(error => {
                        ...
                        alert("An error occurred while fetching the conversion factor.");
                    });
            } else {
                avgWtPerBagInput.value = "";
            }
    ...
    // Update inward weight whenever inward quantity is changed
    if (inwardQtyInput) {
    ...
    // Function to calculate and update inward weight
    function updateInwardWeight() {
        const inwardQty = parseFloat(inwardQtyInput.value) || 0;
        const conversionFactor = parseFloat(avgWtPerBagInput.value) || 0;
        const inwardWt = inwardQty * conversionFactor;

        inwardWtInput.value = inwardWt.toFixed(2); // Update the inward weight field with 2 decimal precision
    }
});
