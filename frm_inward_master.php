            ...
            $row = $select->fetch(PDO::FETCH_ASSOC);
             if($row) {
                    ...
                    if($generator_options) {
                            ...
                            for($i=0;$i<count($fields_names);$i++) {
                                ...
                                switch($fields_types[$i]) {
                                    ...
                                    case "select":
                                        ...
                                        $step = "1";
                                        if (!empty($field_scale[$i]) && $field_scale[$i] > 0) {
                                            $step = "0." . str_repeat("0", $field_scale[$i] - 1) . "1";
                                        }
                                        $step_str = 'step="' . $step . '"';
                                        $min = (!empty($allow_zero) && in_array($fields_names[$i], $allow_zero)) ? 0 : 1;
                                    ...
                                    if ($fields_types[$i] == "select") {
                                        ...
                                        $selected_val = isset(${"val_$fields_names[$i]"}) ? ${"val_$fields_names[$i]"} : "";
                        ...
                        // ✅ Store field IDs for JS conversion
if ($fields_names[$i] == "packing_unit_name") $unit_field_id = $fields_names[$i];
if ($fields_names[$i] == "inward_weight") $weight_field_id = $fields_names[$i];
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const gstTypeInput = document.getElementById("gst_type");
    ...
    if (itemDropdown) {
        itemDropdown.addEventListener("change", function () {
            ...
            if (selectedItem) {
                fetch("classes/cls_inward_master.php?action=fetchGstType&item=" + encodeURIComponent(selectedItem))
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            gstTypeInput.value = data.gst_type;
                        } else {
                            gstTypeInput.value = "";
                            alert(data.message);
                        }
                    ...
                    .catch(error => {
                        ...
                        alert("An error occurred while fetching GST type.");
                    });
            } else {
                gstTypeInput.value = "";
            }
...
<?php if (!empty($unit_field_id) && !empty($weight_field_id)) { ?>
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
$(document).ready(function () {
    const unitField = $('#<?= $unit_field_id ?>');
    const weightField = $('#<?= $weight_field_id ?>');

    function fetchConversionFactor(unit) {
        if (!unit) return;
        $.ajax({
            url: 'get_conversion_factor.php',
            method: 'GET',
            data: { unit: unit },
            success: function (data) {
                if (data.trim() !== '') {
                    weightField.val(parseFloat(data).toFixed(3));
                } else {
                    weightField.val('');
                    console.error("No conversion factor found for unit:", unit);
                }
            },
            error: function () {
                weightField.val('');
                alert("Error fetching conversion factor.");
            }
        });
    }
    ...
    unitField.on('change', function () {
        ...
        fetchConversionFactor(selectedUnit);
    });

    // Optional: fetch factor on page load
    if (unitField.val()) {
        fetchConversionFactor(unitField.val());
    }
});
</script>
<?php } ?>
