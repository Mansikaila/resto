...
{   
    ...
    if ($main_id) {
        ...
        try {
            ...
            $_grid .= "<tbody id=\"tableBody\"><tr id=\"norecords\" class=\"norecords\"><td colspan=\"8\">Error fetching records: " . htmlspecialchars($e->getMessage()) . "</td></tr></tbody></table>";
            return $_grid;
        }
    }
    ...
    try {
        ...
        foreach ($results as &$row) {
            ...
            if ($days_diff < 8) {
                ...
                $inv_day = $days_diff;
                $rent_per_day = $rent / 30;
                $amount = $qty * $rent_per_day * $inv_day;
                $row['invoice_for'] = format_duration($inv_for, 'month');
                $row['invoice_day'] = format_duration($inv_day, 'day');
            } elseif ($days_diff <= 30) {
                $inv_for = 1;
