...
{   
    ...
    public function pageSearch()
    {
        ...
        if ($main_id) {
            // Added rent_invoice_id and weight to select fields
            $sql = "CALL csms1_search_detail('t.description, t.qty, t.unit, t.rate_per_unit, t.amount, t.remark, t.rent_invoice_detail_id, t.rent_invoice_id, t.weight','tbl_rent_invoice_detail t','t.".$main_id_name."=".$main_id."')";
        ...
        if (!empty($result)) {
            foreach ($result as $_rs) {
                ...
                // Now these keys will exist due to select fields above
                $_grid .= "
...
class dal_rentinvoicedetail
{
    public function dbTransaction($_mdl)
    {
        ...
        // Always set a default for manual_rent_per
        if (empty($_mdl->manual_rent_per)) {
            $_mdl->manual_rent_per = 1;
        }
