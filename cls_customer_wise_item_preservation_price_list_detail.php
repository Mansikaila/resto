    ...
    public function dbTransaction($_mdl)
    {
        ...
        try {
            if ($_mdl->detailtransactionmode === 'U') {
                ...
                // Get current packing_unit_id for comparison
                $stmt = $_dbh->prepare("SELECT packing_unit_id FROM tbl_customerwise_item_preservation_price_list_detail 
                ...
                if ($currentData && intval($currentData['packing_unit_id']) !== intval($_mdl->packing_unit_id)) {
                    ...
                    // If packing_unit_id changed, delete and insert new
                    $_dbh->exec("DELETE FROM tbl_customerwise_item_preservation_price_list_detail 
                                 WHERE customer_wise_item_preservation_price_list_detail_id = " . $_mdl->customer_wise_item_preservation_price_list_detail_id);
                    ...
                    // If packing_unit_id same, update rent values only
                    $stmt = $_dbh->prepare("UPDATE tbl_customerwise_item_preservation_price_list_detail
            ...
            // Insert flow (or re-insert after packing_unit_id change)
            if ($_mdl->detailtransactionmode === 'I') {
                $_dbh->exec("SET @p_customer_wise_item_preservation_price_list_detail_id = " .
                    ($_mdl->customer_wise_item_preservation_price_list_detail_id ?? 'NULL'));
