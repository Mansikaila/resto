    ...
    try {
        $stmt = $_dbh->prepare("
            SELECT 
                pum.packing_unit_id, 
                pum.packing_unit_name, 
                COALESCE(cipplm.rent_kg_per_month, ipplm.rent_kg_per_month, '0.00') AS rent_kg_per_month, 
                COALESCE(cipplm.season_rent_per_kg, ipplm.season_rent_per_kg, '0.00') AS season_rent_per_kg,
                CASE 
                    WHEN cipplm.rent_kg_per_month IS NOT NULL OR cipplm.season_rent_per_kg IS NOT NULL THEN 1 
                    ELSE 0 
                END AS is_updated
            FROM tbl_packing_unit_master pum
    ...
    foreach ($data as $unit) {
        try {
            $stmt = $_dbh->prepare("
                ...
                (packing_unit_id, item_id, customer_id, rent_kg_per_month, season_rent_per_kg)
                VALUES (:packing_unit_id, :item_id, :customer_id, :rent_kg_per_month, :season_rent_per_kg)
                ON DUPLICATE KEY UPDATE 
                    rent_kg_per_month = VALUES(rent_kg_per_month),
                    season_rent_per_kg = VALUES(season_rent_per_kg)
            ");
            ...
            $stmt->bindParam(':rent_kg_per_month', $unit['rent_kg_per_month'], PDO::PARAM_STR);
            $stmt->bindParam(':season_rent_per_kg', $unit['season_rent_per_kg'], PDO::PARAM_STR);
            $stmt->execute();
        } catch (PDOException $e) {
