...
//     echo "<pre>";
//    print_r($_SESSION); // ✅ SESSION DEBUGGING
//    echo "</pre>";
//    exit; // 💥 TEMPORARY STOP TO SEE DEBUG OUTPUT
    ...
    foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname) {
        if(isset($_REQUEST[$fieldname])) {
            ...
            if(is_array($_REQUEST[$fieldname])) {
                $_bll->_mdl->{"_".$fieldname} = $_REQUEST[$fieldname];
            } 
            // Handle string values
            else {
                $field = trim($_REQUEST[$fieldname]);
                if($field === '') {
                    if($_bll->_mdl->generator_field_data_type[$i]=="int" || 
            ...
            // Field not set in request
            if($_bll->_mdl->generator_field_data_type[$i]=="int" || 
               $_bll->_mdl->generator_field_data_type[$i]=="bigint" || 
               $_bll->_mdl->generator_field_data_type[$i]=="decimal") {
                $_bll->_mdl->{"_".$fieldname} = 0;
            } else {
                $_bll->_mdl->{"_".$fieldname} = null;
            }
        }
    }
}
...
$_bll->_mdl->_transactionmode =$tmode;
    ...
    $_bll->_mdl->_company_id = $_SESSION['sess_company_id'];
//echo "<pre>";
//echo "Created By: " . $_bll->_mdl->_created_by . "\n";
//echo "Modified By: " . $_bll->_mdl->_modified_by . "\n";
//print_r($_bll->_mdl);
//echo "</pre>";
//exit;
      ...
      // 4️⃣ Inject Debug Print & Prevent Redirect
//    echo "<pre>";
//    echo "Session User ID: " . ($_SESSION['user_id'] ?? 'NOT SET') . "\n";
//    echo "Transaction Mode: " . ($_REQUEST["transactionmode"] ?? 'I') . "\n";
//
//    foreach($_bll->_mdl->generator_fields_names as $i => $fieldname) {
//        $val = $_bll->_mdl->{"_".$fieldname};
//        echo "{$fieldname} => " . var_export($val, true) . "\n";
//    }
//
//    echo "_created_by => " . var_export($_bll->_mdl->_created_by, true) . "\n";
//
//    if (!empty($_bll->_mdl->_array_itemdetail)) {
//        echo "\n--- DETAIL RECORDS ---\n";
//        foreach ($_bll->_mdl->_array_itemdetail as $detailRow) {
//            echo json_encode($detailRow, JSON_PRETTY_PRINT) . "\n";
//        }
//    }
//
//    echo "</pre>";
//  exit; // 👈 Prevent page redirect so you can see the result
                /*** \FOR DETAIL ***/
            $_bll->dbTransaction();
}
