...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms1_search('t2.customer_name, t.inward_id','tbl_inward_master t INNER JOIN tbl_customer_master t2 ON t.customer=t2.customer_id')";
        echo "
...
//    $packing_unit_id = $_GET['packing_unit_id']; // Get the packing_unit_id from the request
//
//    if (!empty($packing_unit_id)) {
//        try {
//            // Query the tbl_packing_unit_master table to fetch the conversion factor
//            $select = $_dbh->prepare("SELECT conversion_factor FROM tbl_packing_unit_master WHERE packing_unit_id = ?");
//            $select->bindParam(1, $packing_unit_id);
//            $select->execute();
//            $row = $select->fetch(PDO::FETCH_ASSOC);
//
//            if ($row) {
//                // Return the conversion factor as JSON
//                echo json_encode([
//                    'success' => true,
//                    'conversion_factor' => $row['conversion_factor'],
//                ]);
//            } else {
//                // Handle case where the packing_unit_id is not found
//                echo json_encode([
//                    'success' => false,
//                    'message' => "Packing unit not found in tbl_packing_unit_master table.",
//                ]);
//            }
//        } catch (PDOException $e) {
...
//        }
//    } else {
//        // Handle missing or invalid packing_unit_id parameter
//        echo json_encode([
//            'success' => false,
//            'message' => "Invalid packing_unit_id provided.",
//        ]);
//    }
//    exit; // End the script
//}
if ($_GET['action'] == 'fetchConversionFactor' && isset($_GET['packing_unit_id'])) {
    require_once '../db_connection.php'; // Adjust this as per your project

    $packing_unit_id = intval($_GET['packing_unit_id']); // sanitization

    $query = "SELECT conversion_factor FROM tbl_packing_unit_master WHERE packing_unit_id = ?";
    $stmt = $db->prepare($query);
    $stmt->bind_param("i", $packing_unit_id);
    $stmt->execute();
